...
[Service]
ExecStart=/usr/bin/screen -S nosana -dm bash -ic "touch /tmp/nosana_start_log.txt; chmod 666 /tmp/nosana_start_log.txt; wget -qO- https://nosana.com/start.sh | bash >> /tmp/nosana_start_log.txt 2>&1; exec bash"
User=$NOSANA_USER
Restart=always
RestartSec=10
